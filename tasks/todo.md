...
- [x] **Limpieza de la comanda de prueba** — ✅ 2026-09-20. Folio 962 borrado a mano en el SQL Editor (verificado antes de borrar, sin restaurar stock a propósito — Javi hace inventario físico el mismo día): `inventory_movements` (3 filas) → `comanda_items` (4 filas) → `comandas` (cascada a `payments` y `comanda_events`). Las 5 verificaciones post-borrado dieron 0 filas — limpio.
- [x] Ajustar propina desde `/admin/folios` — ⏭️ SALTADA (Javi: nunca ha funcionado esa pantalla y nunca se usó; no es una regresión de este cambio, queda fuera de alcance).
- [x] Ajustar stock desde `/inventory` y `/admin/inventory-items` — ✅ 2026-09-20, confirmado en ambas pantallas (prueba 0.2 sobre `adjust_inventory_stock`).
- [x] Botón "Cargar" del Reporte financiero con fechas cambiadas — ✅ 2026-09-20, los 3 contadores (Comandas/Pagos/Movimientos) cambian correctamente al cambiar fechas y darle "Cargar", incluso con un solo día seleccionado (prueba 0.4).
- [x] Paginación de reportes — ✅ 2026-09-20, verificado con control cruzado contra SQL: con rango 10-jun a hoy, el Reporte Semanal muestra Comandas=775, Pagos=775, Movimientos=499 — exactos contra `count(*)` real de las 3 tablas (`select count(*) from comandas where status='paid'` = 775, totales de `payments`/`cash_movements` = 775/499). Sin pérdida ni duplicado de filas (prueba 0.3). Nota de proceso: la primera comparación (834 vs 773 en "Comandas") pareció un bug pero fue un error de comparación mío — 834 es TODAS las comandas, la pantalla solo cuenta `status='paid'` — quedó resuelto al corregir el filtro de la comparación, no al tocar código.
- [x] Utilidad neta de agosto en `MonthlyReportPage` — ✅ RESUELTO 2026-09-20, sin bug. La discrepancia inicial (-$60k en vez de subir a -$46k) se debía al COGS: no está congelado, usa `reference_cost` de HOY para cualquier mes (decisión de diseño ya documentada en el código). Se verificó producto por producto con SQL replicando `getProductSalesForPeriod` — la suma dio exacto $38,161.37 contra pantalla. Javi confirmó que justo le acababa de poner costo a "Refresco" (807 unidades sin costear en agosto) — eso solo explica el salto de $13,988.46 en el COGS entre la primera lectura y la segunda. El fix de 0.6 (propina) queda confirmado 100% correcto: gastos operativos = $129,462.90 exacto a mano, fórmula consistente ($107,422.70 − COGS − $129,462.90 = utilidad mostrada).
