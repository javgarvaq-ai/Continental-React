...
// Returns the ISO string for the start of the current "operational day" in
// Mexico timezone (-06:00). The bar's day runs past midnight (shifts often
// close around 2am), so the cutover is 06:00 local instead of 00:00 — before
// 6am, "today" still refers to the previous calendar date. This keeps the
// dashboard aligned with the currently open shift instead of splitting it at
// midnight.
// Do NOT use d.toISOString() here: it converts to UTC and shifts the boundary,
...
function startOfToday() {
    ...
    return `${y}-${m}-${day}T06:00:00-06:00`
}
...
//   Pass shift.opened_at so the Dashboard always reflects the current shift.
export async function getTodayPaymentStats(since) {
    const { data, error } = await supabase
        .from('payments')
        .select('total_paid, tip_amount, efectivo, tarjeta, transferencia, comanda_id')
        .gte('created_at', since || startOfToday())
...
// Includes active comanda_items so callers can compute the running total
// (final_total is only set at payment time, not while the comanda is open).
export async function getOpenTables() {
    const { data, error } = await supabase
        .from('comandas')
        .select(`
            id, status, opened_at, folio, final_total, customer_name,
            units(name),
            customers(name),
            comanda_items!comanda_items_comanda_id_fkey(quantity, unit_price, status, is_free_mixer, is_free_benefit)
        `)
...
export async function getTopProductsToday(since) {
    ...
    if (e1 || !payments || payments.length === 0) return { data: [], error: e1 }

    const ids = [...new Set(payments.map(p => p.comanda_id).filter(Boolean))]
...
// ── Membership activations this shift ────────────────────────
// `since` — ISO string start of period (defaults to startOfToday()).
export async function getMembershipStatsToday(since) {
